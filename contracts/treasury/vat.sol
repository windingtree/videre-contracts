...
contract Vat {
  ...
  // --- Fungibility
  function slip(address usr, address token, int256 wad) external auth {
    gem[usr][token] = _add(gem[usr][token], wad);
  }

  function slip(bytes32 who, address token, int256 wad) external auth {
    bag[who][token] = _add(bag[who][token], wad);
  }

  /// @dev Normal accounts
  function flux(address src, address dst, address token, uint256 wad) external {
  ...
  // --- Voucher Handling
  function deal(bytes32 v, address u, address t, uint256 wad, uint256 fee) external auth {
    // system is live
  ...
  // --- Voucher fungibility
  function swap(bytes32 v, address src, address dst, address t, uint256 wad, uint256 fee) external auth {
    // system is live
    require(live == 1, "Vat/not-live");
  ...
  // --- Voucher settlement
  function move(bytes32 src, bytes32 dst, address t, uint256 wad, uint256 fee) external auth {
    // system is live
    require(live == 1, "Vat/not-live");
  ...
  function suck(bytes32 src, address dst, address t, uint256 wad, uint256 fee) external auth {
    // system is live
    require(live == 1, "Vat/not-live");
    // protocol fee isn't higher than amount
    require(fee <= wad, "Vat/fee-too-high");
