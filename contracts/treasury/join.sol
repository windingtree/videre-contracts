...
interface VatLike {
  ...
  function slip(address,address,int) external;
}
...
contract GemJoin is EIP712 {
  ...
  constructor(string memory name, string memory version, address vat_, address gem_) EIP712(name, version){
    wards[msg.sender] = 1;
    live = 1;
    vat = VatLike(vat_);
    gem = GemLike(gem_);
  ...
  function _join(address src, address dst, uint256 wad) internal {
    require(live == 1, "GemJoin/not-live");
    require(int(wad) >= 0, "GemJoin/overflow");
    vat.slip(dst, address(gem), int(wad));
  ...
  function permitJoin(address usr, uint256 wad, EIP20Permit calldata permit, bytes calldata sig) external {
    require(
      SignatureChecker.isValidSignatureNow(
        permit.owner,
        hash(GemJoinPermit(usr, wad, abi.encode(permit))),
        sig
      ),
      "GemJoin/invalid-sig"
    );
  ...
  function exit(address usr, uint wad) external {
    ...
    vat.slip(msg.sender, address(gem), -int(wad));
