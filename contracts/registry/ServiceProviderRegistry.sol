...
contract ServiceProviderRegistry is IServiceProviderRegistry, AccessControl, AbstractWhitelistExpiry {
  ...
  function can(bytes32 which, Role what, address who) public view returns (bool) {
    // @dev write role can be admins, apis, or managers.
    if (what == Role.WRITE) {
      return (
        hasRole(_calcRole(which, Role.WRITE), who) ||
  ...
  constructor() 
    AbstractWhitelistExpiry(block.timestamp + WHITELIST_EXPIRY) 
  {
  ...
  /// @inheritdoc IServiceProviderRegistry
  function enroll(bytes32 salt, string calldata dataURI) external onlyWhitelist override returns (bytes32 provider) {
    provider = keccak256(abi.encode(
      salt, _msgSender()
    ));
  ...
  /// @inheritdoc IServiceProviderRegistry
  function file(bytes32 which, bytes32 what, string calldata data) external onlyWrite(which) override {
    if (what == "dataURI") {
  ...
  function file(bytes32 which, bytes32 what, uint256 data) external onlyAdmin(which) {
