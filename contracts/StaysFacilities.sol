...
contract StaysFacility is Context, AccessControl, EIP712 {
  ...
  // constants
  bytes32 constant BID_TYPE = keccak256("Bid(bytes32 facilityHash,bytes32 spaceHash,bytes32 askDigest,bytes32 termsHash,uint64 expiry,address costToken,uint256 costAtoms)");
  ...
  event Deal(bytes32 indexed facility, bytes32 indexed spaceHash, bytes32 indexed askDigest, bytes32 termsHash, address costToken, uint256 costAtoms);
  ...
  function registerFacility(
    bytes32 salt,
    Facility calldata _newFacility
  ) public returns (bytes32 facilityId) {
  ...
  function registerSpace(
    bytes32 facilityHash,
    Space calldata _newSpace
  ) public onlyWriteAccess(facilityHash) returns (bytes32 spaceId) {
  ...
  function deal(Bid calldata bid, bytes calldata signature) external payable {
    ...
    if (bid.costToken == address(0)) {
      ...
      require(IERC20(bid.costToken).allowance(_msgSender(), address(this)) >= bid.costAtoms, "facilities/insufficient-erc20-allowance");
    }
  ...
  function hasWriteAccess(bytes32 which, address who) public view returns (bool) {
    return (
      hasRole(calcRole(which, Role.MANAGER), who) ||
