...
describe('LineRegistry', function () {
  ...
  context('Protocol Governance', async () => {
    ...
    it('rely is guarded', async () => {
      await expect(alice.lRegistry.rely(alice.address)).to.be.revertedWith('registry/not-authorized')
    })
    it('can rely', async () => {
    ...
    it('deny is guarded', async () => {
      await expect(alice.lRegistry.deny(deployer.address)).to.be.revertedWith('registry/not-authorized')
    })
    it('can deny', async () => {
      await deployer.lRegistry.rely(alice.address)
      const status = await deployer.lRegistry.wards(alice.address)
      await expect(alice.lRegistry.deny(alice.address)).to.not.be.reverted
      expect(await alice.lRegistry.wards(alice.address)).to.be.eq(status.sub(1))
    })
    context('#file', async () => {
      it('file (uint256 and address) is guarded', async () => {
        await expect(
          alice.lRegistry['file(bytes32,bytes32,address)'](
        ...
        await expect(
          alice.lRegistry['file(bytes32,bytes32,uint256)'](constants.HashZero, constants.HashZero, 1)
        ).to.be.revertedWith('registry/not-authorized')
      })
      it('can file lines and their cuts', async () => {
        ...
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,address)'](utils.formatBytes32String('terms'), line, termsAddr)
        ).to.not.be.reverted
        // line now exists
        ...
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,uint256)'](
            utils.formatBytes32String('cut'),
            line,
            BigNumber.from(10).pow(27).add(1)
          )
        ...
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,address)'](
            utils.formatBytes32String('terms'),
            line,
            constants.AddressZero
          )
      ...
      it('can file self register', async () => {
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,uint256)'](
            utils.formatBytes32String('self_register'),
            constants.HashZero,
            1
          )
        ).to.not.be.reverted
      ...
      it('can file a service provider registry', async () => {
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,address)'](
            utils.formatBytes32String('service_provider_registry'),
            constants.HashZero,
            constants.AddressZero
          )
      ...
      it('cannot specify cut for non-existent line', async () => {
        await expect(
          deployer.lRegistry['file(bytes32,bytes32,uint256)'](
            utils.formatBytes32String('cut'),
            utils.formatBytes32String('random-line'),
            1
          )
    ...
    context('#hope / #nope', async () => {
      ...
      it('hope works for permissioned address', async () => {
        await expect(deployer.lRegistry.hope(line, serviceProviderId)).to.not.be.reverted
      ...
      it('nope is guarded', async () => {
        ...
        await expect(alice.lRegistry.nope(line, serviceProviderId)).to.be.revertedWith('registry/not-authorized')
        expect(await deployer.lRegistry.can(line, serviceProviderId)).to.be.eq(true)
      })
      it('nope works for permissioned address', async () => {
        await deployer.lRegistry.hope(line, serviceProviderId)
        await expect(deployer.lRegistry.nope(line, serviceProviderId)).to.not.be.reverted
        expect(await deployer.lRegistry.can(line, serviceProviderId)).to.be.eq(false)
      })
    })
    context('#register / #deregister', async () => {
      ...
      it('register a service provider for an economic line', async () => {
        await deployer.lRegistry['file(bytes32,bytes32,address)'](utils.formatBytes32String('terms'), line, terms)
        await expect(alice.lRegistry.register(line, serviceProviderId)).to.be.revertedWith('registry/not-authorized')
        expect(await alice.lRegistry.can(line, serviceProviderId)).to.be.eq(false)
        await deployer.lRegistry['file(bytes32,bytes32,uint256)'](
          utils.formatBytes32String('self_register'),
          constants.HashZero,
          0
        )
        await expect(bob.lRegistry.register(line, serviceProviderId)).to.be.revertedWith('registry/not-authorized')
        await deployer.lRegistry['file(bytes32,bytes32,uint256)'](
          utils.formatBytes32String('self_register'),
          constants.HashZero,
          1
        ...
        await expect(bob.lRegistry.register(line, utils.formatBytes32String('not-provider-id'))).to.be.revertedWith(
          'registry/not-authorized'
        )
        // cannot register service provider for a line that doesn't exist
        await expect(
          bob.lRegistry.register(utils.formatBytes32String('not-a-line'), serviceProviderId)
        ).to.be.revertedWith('registry/no-such-line')
        expect(await bob.lRegistry.can(utils.formatBytes32String('not-a-line'), serviceProviderId)).to.be.eq(false)
        await expect(bob.lRegistry.register(line, serviceProviderId)).to.not.be.reverted
        expect(await bob.lRegistry.can(line, serviceProviderId)).to.be.eq(true)
      })
      ...
      it('can deregister a service provider from an economic line', async () => {
        ...
        await expect(manager.lRegistry.deregister(line, serviceProviderId)).to.be.revertedWith(
          'registry/not-authorized'
        )
        await expect(bob.lRegistry.deregister(line, serviceProviderId)).to.not.be.reverted
      })
    })
  })
})
