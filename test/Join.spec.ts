...
describe('GemJoin', function () {
  ...
  context('Protocol Governance', async () => {
    ...
    it('rely is guarded', async () => {
      await expect(alice.join.rely(alice.address))
        .to.be.revertedWith('GemJoin/not-authorized')
    })
    it('can rely', async () => {
    ...
    it('deny is guarded', async () => {
      await expect(alice.join.deny(deployer.address))
        .to.be.revertedWith('GemJoin/not-authorized')
    })
    it('can deny', async () => {
      await deployer.join.rely(alice.address)
      const status = await deployer.join.wards(alice.address)
      await expect(alice.join.deny(alice.address))
        .to.not.be.reverted
  ...
  context('#join', async () => {
    ...
    it('cannot join when not live', async () => {
      ...
      await expect(alice.join.join(
        alice.address,
        utils.parseEther('100')
      )).to.be.revertedWith('GemJoin/not-live')
    })
    ...
    it('protects against int256 overflow', async () => {
      ...
      )).to.be.revertedWith('GemJoin/overflow')
    })

    it('fails if not enough allowance', async () => {
      await expect(bob.join.join(
        bob.address,
        utils.parseEther('100')
      )).to.be.reverted
    })
    ...
    it('can join collateral', async () => {
      ...
      expect(await alice.erc20.balanceOf(alice.address))
        .to.be.eq(aliceBalance.sub(AMOUNT.mul(2)))
      ...
      expect(await bob.vat.gems(bob.address, bob.erc20.address))
        .to.be.eq(bobVatBalance.add(AMOUNT))
    })
  })
  ...
  context('#exit', async () => {
    ...
    it('protects against overflow', async () => {
      await expect(alice.join.exit(
        alice.address,
        BigNumber.from('2').pow(255).add(1)
      )).to.be.revertedWith('GemJoin/overflow')
    })
    ...
    it('can exit collateral', async () => {
      ...
      await alice.join.exit(
        alice.address,
        utils.parseEther('500')
      )

      expect(await alice.erc20.balanceOf(alice.address))
        .to.be.eq(aliceBalance.add(utils.parseEther('500')))
      expect(await alice.vat.gems(alice.address, alice.erc20.address))
        .to.be.eq(aliceVatBalance.sub(utils.parseEther('500')))
      ...
      expect(await alice.vat.gems(alice.address, alice.erc20.address))
        .to.be.eq(0)
      expect(await bob.erc20.balanceOf(bob.address))
        .to.be.eq(bobBalance.add(utils.parseEther('500')))
    })
  })
})
